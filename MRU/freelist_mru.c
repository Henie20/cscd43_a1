...
StrategyGetBuffer(BufferAccessStrategy strategy, uint32 *buf_state, bool *from_ring)
{
	...
	*/
	...
	// BEGIN NEWCODE
	// MRU Algorithm
	BufferDesc *min_buf = NULL;
	time_t min_time = -1;
	
	// loop through all buffers
	for (int i = 0; i < NBuffers; i++) {
		BufferDesc *buffer = &BufferDescriptors[i].bufferdesc;
		local_buf_state = LockBufHdr(buffer);
		time_t time = buffer->access_time;
		int refcount = BUF_STATE_GET_REFCOUNT(local_buf_state);

		// check for most recently used
		if (refcount == 0 && time > min_time) {
			min_buf = buffer;
			min_time = time;
		}
		UnlockBufHdr(buffer, local_buf_state);
	}
	if (min_buf != NULL) 
		return min_buf;
	// END NEWCODE
